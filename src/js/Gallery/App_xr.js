...
export default class App
{
  ...
  draw(direction)
  {
    ...
    {
      //レイキャスター
      this.mouse3D = new THREE.Vector2(direction.x, direction.y);
  ...
  addBorad()
  {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      this.boardPosRadius = 150;

      // this.board = new PosterBoard(parameter);
      this.board = new PosterBoard_xr(parameter);
      ...
      {
        this.rootScene.appendChild(mesh.entity);
      }
  ...
  createScene() {
    ...
    if (!isVR()) {
      document.getElementById(SELECTORS.Gallery).appendChild(this.renderer.domElement);
    }
  }
